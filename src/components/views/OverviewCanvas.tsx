...
import { Globe, Newspaper, Radio, Camera, ShieldAlert, Sparkles, Activity, Layers, Play, Compass } from 'lucide-react';
...
export default function OverviewCanvas({
  ...
  const [activeCenterView, setActiveCenterView] = useState<'RADAR' | 'NEWS' | 'RECON'>('RADAR');
  ...
  return (
    <div className="space-y-4 select-none font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        {/* LEFT / CENTER (7 COLS): MULTI-LAYER TACTICAL WORKSPACE (RADAR / NEWS / RECON) */}
          ...
          <div className="flex items-center justify-between px-3 py-1.5 rounded bg-[#070b10] border border-white/10 text-xs">
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                {[
                  { id: 'RADAR', label: 'Tactical Radar COP', icon: Compass },
            ...
            <div className="flex items-center gap-2">
              ...
              {onNavigateToTab && activeCenterView !== 'RADAR' && (
                <button
                  onClick={() =>
                    onNavigateToTab(activeCenterView === 'NEWS' ? 'news' : 'recon')
                  }
                ...
                >
                  EXPAND [{activeCenterView === 'NEWS' ? 'N' : 'R'}] ↗
                </button>
              )}
            </div>
          </div>
