...
export default function EventReconMedia({ event }: EventReconMediaProps) {
  ...
  return (
    <div className="p-4 bg-white/[0.035] backdrop-blur-md rounded-xl border border-[#526a27]/40 space-y-4 font-mono shadow-2xl">
      {/* 1. REAL SOURCE CITATION & LIVE API BADGE */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] bg-emerald-950 border border-emerald-600 text-emerald-300 font-bold px-2 py-0.5 rounded-lg flex items-center gap-1">
          ...
          <h4 className="font-heading font-bold text-sm text-slate-100">{source.name}</h4>
        ...
        <div className="flex flex-col sm:items-end gap-1.5">
          <a
            ...
            className="px-3 py-1 bg-[#a4c639]/10 backdrop-blur-md border border-[#3f5220] hover:bg-[#a4c639]/14 text-[#bcd94f] rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors"
          >
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
          <span className="text-slate-200 font-bold flex items-center gap-1.5">
            <Camera className="w-4 h-4 text-[#a4c639]" /> HIGH-CLARITY SATELLITE IMAGERY (ESRI ORBITAL CDN)
          </span>
          ...
          <div className="flex items-center gap-1.5 text-[10px]">
            <button
              ...
              className={`px-2 py-0.5 rounded-lg border font-bold transition-all ${
                !useHdExport && zoomLevel === 17
                  ? 'bg-[#a4c639]/10 backdrop-blur-md border-[#526a27] text-[#bcd94f]'
                  : 'bg-white/[0.05] backdrop-blur-md border-white/10 text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded-lg border font-bold transition-all ${
                useHdExport
                  ? 'bg-amber-500/12 backdrop-blur-md border-amber-500 text-amber-300'
                  : 'bg-white/[0.05] backdrop-blur-md border-white/10 text-slate-400 hover:text-slate-200'
              }`}
            >
        ...
        <div className="relative w-full h-64 bg-[#070b14] rounded-lg overflow-hidden border border-white/10 shadow-2xl group">
          {/* Animated Loading Skeleton */}
          {!imageLoaded && !imageError && (
            <div className="absolute inset-0 z-20 bg-white/[0.035] backdrop-blur-md flex flex-col items-center justify-center text-[#a4c639] text-xs gap-2">
              <Loader2 className="w-7 h-7 animate-spin text-[#a4c639]" />
              <div className="font-heading font-bold">LOADING HIGH-CLARITY SATELLITE TILES...</div>
            </div>
          )}
          ...
          {!imageError ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-white/[0.035] backdrop-blur-md text-slate-400 text-xs p-4 text-center">
              <Camera className="w-8 h-8 text-[#a4c639] mb-2 opacity-50" />
              <div>High-Res Satellite Image Loaded for ({lat.toFixed(4)}°, {lng.toFixed(4)}°)</div>
            </div>
          )}

          {/* TACTICAL HUD OVERLAY */}
          <div className="absolute inset-0 pointer-events-none border border-[#526a27]/30 m-2 rounded-lg z-10">
            {/* Corner Markers */}
            <div className="absolute top-0 left-0 w-3.5 h-3.5 border-t-2 border-l-2 border-[#a4c639]"></div>
            <div className="absolute top-0 right-0 w-3.5 h-3.5 border-t-2 border-r-2 border-[#a4c639]"></div>
            <div className="absolute bottom-0 left-0 w-3.5 h-3.5 border-b-2 border-l-2 border-[#a4c639]"></div>
            <div className="absolute bottom-0 right-0 w-3.5 h-3.5 border-b-2 border-r-2 border-[#a4c639]"></div>
            ...
            {isPlayingVideo && (
              <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#a4c639]/15 to-transparent h-12 animate-pulse pointer-events-none"></div>
            )}
          </div>

          {/* SATELLITE HUD METADATA BANNER */}
          <div className="absolute top-2 left-2 z-10 bg-white/[0.035] backdrop-blur-md px-2.5 py-1 rounded-lg text-[10px] text-[#bcd94f] font-bold border border-white/10">
            ESRI ORBITAL HIGH-RES ● ZOOM {zoomLevel} | LAT {lat.toFixed(4)}° N | LNG {lng.toFixed(4)}° E
          </div>

          <div className="absolute bottom-2 left-2 right-2 z-10 bg-white/[0.035] backdrop-blur-md p-2 rounded-lg text-[10px] text-slate-200 border border-white/10 flex items-center justify-between">
            <span>HIGH-DEFINITION ORBITAL SATELLITE PASS</span>
            <a
              href={hdExportUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#a4c639] font-bold hover:underline flex items-center gap-1"
            >
      ...
      <div className="border-t border-white/10 pt-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className="px-3 py-1 bg-emerald-950 border border-emerald-700 hover:bg-emerald-900 text-emerald-300 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors disabled:opacity-50"
          >
        ...
        {liveApiResponse && (
          <div className="bg-white/[0.035] backdrop-blur-md p-3 rounded-lg border border-emerald-900/60 max-h-40 overflow-y-auto">
            <div className="text-[10px] text-emerald-400 font-bold mb-1">
              ✓ RESPONSE RETURNED FROM {source.apiUrl}
            </div>
            <pre className="text-[11px] text-[#bcd94f] font-mono leading-relaxed overflow-x-auto">
